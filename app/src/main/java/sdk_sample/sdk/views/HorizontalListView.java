...
public class HorizontalListView extends AdapterView<ListAdapter> {
    ...
    private void addAndMeasureChild(View child, int viewPos) {
        ...
        child.measure(MeasureSpec.makeMeasureSpec(this.getWidth(), -2147483648), MeasureSpec.makeMeasureSpec(this.getHeight(), -2147483648));
    }
    ...
    private void fillListRight(int rightEdge, int dx) {
        while(true) {
            try {
                if(rightEdge + dx < this.getWidth() && this.mRightViewIndex < this.mAdapter.getCount()) {
                    View e = this.mAdapter.getView(this.mRightViewIndex, (View)this.mRemovedViewQueue.poll(), this);
                    this.addAndMeasureChild(e, -1);
                    rightEdge += e.getMeasuredWidth();
    ...
    private void removeNonVisibleItems(int dx) {
        ...
        for(child = this.getChildAt(this.getChildCount() - 1); child != null && child.getLeft() + dx >= this.getWidth(); child = this.getChildAt(this.getChildCount() - 1)) {
            this.mRemovedViewQueue.offer(child);
            this.removeViewInLayout(child);
            --this.mRightViewIndex;
        }
