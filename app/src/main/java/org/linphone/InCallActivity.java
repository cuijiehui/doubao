...
import com.xinspace.csevent.monitor.adapter.VpDataAdapter;
...
public class InCallActivity extends FragmentActivity implements OnClickListener {
	...
	private TextView pause, dialer,  options, addCall, transfer, conference;
	private ImageView hangUp , micro , video, speaker;
	...
	private RollPagerView vp_video;
	private VpDataAdapter adapter;
	private List<VideoAdBean> allList = new ArrayList<>();
...
//				case 200:
//					if (msg.obj != null){
//						allList.addAll((Collection<? extends VideoAdBean>) msg.obj);
...
//	};
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		intent = getIntent();
		if (intent != null){
			sipEvent = (SipEvent) intent.getSerializableExtra("sipEvent");
		}
...
//		GetDataBiz.GET_INCAll_VD_DATA(new HttpRequestListener() {
//			@Override
...
//					List<VideoAdBean> list = new ArrayList<VideoAdBean>();
//					for (int i = 0 ; i < dataJsonArray.length() ; i++ ){
//						JSONObject vpJsonObject = dataJsonArray.getJSONObject(i);
//						VideoAdBean bean = new VideoAdBean();
//						bean.setId(vpJsonObject.getString("id"));
//						bean.setPic(vpJsonObject.getString("pic"));
//						list.add(bean);
//					}
...
//	}
	...
	private boolean isVideoEnabled(LinphoneCall call) {
		if(call != null){
			Log.i("www" , "isVideoEnabled返回值" + call.getCurrentParamsCopy().getVideoEnabled());
	...
	private void initUI() {
		...
		container.setBackgroundResource(R.drawable.opendoor_bg);
		...
		video.setEnabled(false); // 打开视频的按钮
...
//		speaker.setEnabled(false);
	...
	private void enabledOrDisabledVideo(final boolean isVideoEnabled) {
		...
		Log.i("www" , "isVideoEnabled" + isVideoEnabled);
...
//		Hiding controls to let displayVideoCallControlsIfHidden add them plus the callback
		...
		bundle.putSerializable("sipEvent" , sipEvent);
