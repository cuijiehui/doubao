package com.xinspace.csevent.baskorder.view;
...
public class ScaleTopHeaderView extends ImageView {
	private Float mCropYCenterOffsetPct;
	private Float mCropXCenterOffsetPct;

	public ScaleTopHeaderView(Context context) {
		super(context);
		setScaleType(ScaleType.MATRIX);
	}

	public ScaleTopHeaderView(Context context, AttributeSet attrs) {
		super(context, attrs);
		setScaleType(ScaleType.MATRIX);
	}
	...
	private void myConfigureBounds() {
		if (this.getScaleType() == ScaleType.MATRIX) {
			 ...
			 * 
			 * Excerpt from their source: } else if (ScaleType.CENTER_CROP ==
			 * mScaleType) { mDrawMatrix = mMatrix;
			 * 
			 * float scale; float dx = 0, dy = 0;
			 * 
			 * if (dwidth * vheight > vwidth * dheight) { scale = (float)
			 * vheight / (float) dheight; dx = (vwidth - dwidth * scale) * 0.5f;
			 * } else { scale = (float) vwidth / (float) dwidth; dy = (vheight -
			 * dheight * scale) * 0.5f; }
			 * 
			...
			if (d != null) {
				...
				if (dwidth * vheight > vwidth * dheight) {
					float cropXCenterOffsetPct = mCropXCenterOffsetPct != null ? mCropXCenterOffsetPct.floatValue()
							: 0.5f;
					scale = (float) vheight / (float) dheight;
					dx = (vwidth - dwidth * scale) * cropXCenterOffsetPct;
				} else {
					float cropYCenterOffsetPct = mCropYCenterOffsetPct != null ? mCropYCenterOffsetPct.floatValue()
							: 0f;
